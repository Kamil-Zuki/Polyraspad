...
public class S3MediaStorageService : IMediaStorageService
{
    ...
    public async Task<ReaderLibraryBookRecord> SaveReaderLibraryBookAsync(Guid userId, string projectId, ReaderLibraryBookRecord book, CancellationToken cancellationToken = default)
    {
        ...
        if (existing != null)
        {
            ...
            book.CoverImageUrl = string.IsNullOrWhiteSpace(book.CoverImageUrl) ? existing.CoverImageUrl : book.CoverImageUrl;
            book.AudioUrl = string.IsNullOrWhiteSpace(book.AudioUrl) ? existing.AudioUrl : book.AudioUrl;
            book.CefrLevel = string.IsNullOrWhiteSpace(book.CefrLevel) ? existing.CefrLevel : book.CefrLevel;
            book.Summary = string.IsNullOrWhiteSpace(book.Summary) ? existing.Summary : book.Summary;
        }
        ...
        var updatedBooks = books
            .Select(book => book.CollectionId == collectionId.ToString()
                ? new ReaderLibraryBookRecord
                {
                    ...
                    LastPageNumber = book.LastPageNumber,
