...
- **File:** `detroit-tdd-orchestrator.md` · **name:** `detroit-tdd-orchestrator`
- **Role:** Coordinator: composes tasks in `.cursor/tasks/`, delegates to workers, manages TDD stages, and launches architect / infra-verifier when necessary.
...
- **File:** `worker.md` · **name:** `worker`
- **Role:** Executor: performs red/green/refactor for a given task, writes code and tests, returns a structured report.
...
- **Role:** Reviewer (read-only): verifies architectural layers, ensures no transport bleeding, checks transactions and contracts. Runs after green/refactor.
...
- **File:** `infra-verifier.md` · **name:** `infra-verifier`
- **Role:** Diagnoses Docker, RabbitMQ, Redis, and ports; resolves Testcontainers and integration test failures.
...
2. **01** — Worker executes the specific assigned TDD stage (e.g., _only_ red, or _only_ green) and returns the result.
