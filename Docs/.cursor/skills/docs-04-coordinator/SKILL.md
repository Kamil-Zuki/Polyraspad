...
description: Plans and orchestrates filling folder 04 for a microservice — manifest, group alignment from 01, write order, delegation to writer/verifier. Use when generating entire 04, batch 04 work, or user invokes @docs-04-coordinator.
---

# Microservice Docs — 04 Coordinator

Orchestrates **folder `04 - Бекенд, API и Контракты`** for a target service. Does **not** duplicate block templates — those live in `.cursor/rules/docs-folder-04-*.mdc`.
...
4. Delegate writing in **write order** (see `docs-04-write` → `write-order.md`):
...
- Rules G2: `.cursor/rules/docs-folder-04-coordinator.mdc`
