...
Detailed steps: `.cursor/rules/steos-docs-staging-0103.mdc`.
...
| `Шаблон документации микросервиса STEOS/` | Short copies — markdown layout and table patterns only |
| `(Done)/*`                                | Do not rewrite without explicit user request           |

**Auth — не источник содержания для других сервисов:**

- Do **not** copy, paraphrase, or weave in Auth **text**: SR descriptions, scenarios, metaphors, DTO fields, endpoints, Redis keys, Rabbit flows, domain terms (sessions, OIDC, Phantom Token, STEOS ID, etc.).
...
Область `01`↔`03` задана rule `steos-docs-staging-0103.mdc` — в ISSUE не дублировать. Стиль и навигация ISSUE (якорь **SR-ID** / **Entity.field**, язык для человека): `steos-docs-staging-issues.mdc`. Другие области (`04`, `02`): `.cursor/skills/steos-docs-04-verify/issue-template.md`.
