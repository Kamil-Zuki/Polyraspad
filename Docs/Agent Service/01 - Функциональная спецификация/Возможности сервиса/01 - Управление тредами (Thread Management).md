...
4. **Сохранение Override:** если был передан `SystemPromptOverride` (например, для диагностического теста), он сохраняется в БД для этого треда и будет подставляться вместо стандартного "Study Copilot" при каждом запуске агента (Run).
5. **Ответ клиенту:** gRPC возвращает новый thread id — UI переходит к пустому диалогу.

Таким образом, каждая новая беседа начинается с изолированного thread container, привязанного к project и user. Контейнер может иметь кастомные настройки агента через Override.
