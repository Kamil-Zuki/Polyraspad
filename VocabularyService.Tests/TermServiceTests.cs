...
public class TermServiceTests
{
    ...
    public async Task ListProjectTermsAsync_FilterNew_ReturnsOnlyNewTerms()
    {
        ...
        var list = await sut.ListProjectTermsAsync(userId, projectId, "NEW", null, null, 1, 50, default);
    ...
    [Fact]
    public async Task ListProjectTermsAsync_PageSizeOne_ReturnsPagedResult()
    {
        var dbName = Guid.NewGuid().ToString();
        var userId = Guid.NewGuid();
        var projectId = Guid.NewGuid();
        ...
        var first = await sut.ListProjectTermsAsync(userId, projectId, null, null, null, 1, 1, default);
        first.Items.Should().HaveCount(1);

        var second = await sut.ListProjectTermsAsync(userId, projectId, null, null, null, 2, 1, default);
